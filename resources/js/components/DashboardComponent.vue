<template>
    <div class="container">
                ...
                <h2 class="accordion-header" :id="'heading' + client.id">
                     <button class="accordion-button collapsed" type="button" @click="toggleClient(client.id)">
                        ...
                        <div class="ms-auto pe-3">
                            ...
                            <button class="btn btn-outline-danger btn-sm" @click.stop="handleDeleteClient(client)">Удл.</button>
                        </div>
                    </button>
                </h2>
        ...
        <div class="modal fade" id="clientModal" tabindex="-1" ref="clientModal">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form @submit.prevent="handleSubmitClient">
                        <div class="modal-header">
                            <h5 class="modal-title">{{ isEditMode ? 'Редактировать клиента' : 'Новый клиент' }}</h5>
                            <button type="button" class="btn-close" @click="closeClientModal"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label">Имя</label>
                                <input type="text" class="form-control" v-model="clientForm.name" required :class="{ 'is-invalid': formErrors.name }">
                                <div v-if="formErrors.name" class="invalid-feedback">{{ formErrors.name[0] }}</div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Email</label>
                                <input type="email" class="form-control" v-model="clientForm.email" :class="{ 'is-invalid': formErrors.email }">
                                <div v-if="formErrors.email" class="invalid-feedback">{{ formErrors.email[0] }}</div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Телефон</label>
                                <input type="text" class="form-control" v-model="clientForm.phone" :class="{ 'is-invalid': formErrors.phone }">
                                <div v-if="formErrors.phone" class="invalid-feedback">{{ formErrors.phone[0] }}</div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Ставка по умолчанию (RUB/час)</label>
                                <input type="number" class="form-control" v-model.number="clientForm.default_rate" step="0.01" placeholder="Не обязательно" :class="{ 'is-invalid': formErrors.default_rate }">
                                <div v-if="formErrors.default_rate" class="invalid-feedback">{{ formErrors.default_rate[0] }}</div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" @click="closeClientModal">Отмена</button>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <form @submit.prevent="handleSubmitProject">
                        <div class="modal-header">
                            <h5 class="modal-title">{{ isEditMode ? 'Редактировать проект' : 'Новый проект' }}</h5>
                            <button type="button" class="btn-close" @click="closeProjectModal"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label">Название</label>
                                <input type="text" class="form-control" v-model="projectForm.title" required :class="{ 'is-invalid': formErrors.title }">
                                <div v-if="formErrors.title" class="invalid-feedback">{{ formErrors.title[0] }}</div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Описание</label>
                                <textarea class="form-control" v-model="projectForm.description" rows="3" :class="{ 'is-invalid': formErrors.description }"></textarea>
                                <div v-if="formErrors.description" class="invalid-feedback">{{ formErrors.description[0] }}</div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" @click="closeProjectModal">Отмена</button>
                            <button type="submit" class="btn btn-primary">Сохранить</button>
                        </div>
                    </form>
                </div>
        ...
        <!-- Модальное окно Добавления Записи Времени Вручную -->
...
const fetchSummary = async () => { summaryLoading.value = true; try { const response = await axios.get('/api/dashboard-summary'); Object.assign(summary, response.data); } catch (error) { console.error("Не удалось загрузить сводку:", error); toast.error("Не удалось загрузить сводку"); } finally { summaryLoading.value = false; } };
...
// --- Методы для Клиентов ---
const openClientModal = (client = null) => { isEditMode.value = !!client; Object.assign(clientForm, client || { id: null, name: '', email: '', phone: '', default_rate: null }); formErrors.value = {}; clientModalInstance?.show(); };
const closeClientModal = () => clientModalInstance?.hide();
const handleSubmitClient = async () => { try { if (isEditMode.value) { await axios.put(`/api/clients/${clientForm.id}`, clientForm); toast.success('Клиент успешно обновлен!'); } else { await axios.post('/api/clients', clientForm); toast.success('Клиент успешно создан!'); } closeClientModal(); await fetchClients(); } catch (error) { handleError(error, 'клиента'); } };
const handleDeleteClient = async (client) => { if (confirm(`Удалить клиента "${client.name}" и все его проекты?`)) { try { await axios.delete(`/api/clients/${client.id}`); toast.success(`Клиент "${client.name}" удален.`); await fetchClients(); fetchSummary(); } catch (error) { handleError(error, 'клиента'); } } };

// --- Методы для Проектов ---
const openProjectModal = (project = null, clientId = null) => { isEditMode.value = !!project; Object.assign(projectForm, project || { id: null, client_id: clientId, title: '', description: '' }); formErrors.value = {}; projectModalInstance?.show(); };
const closeProjectModal = () => projectModalInstance?.hide();
const handleSubmitProject = async () => { try { if (isEditMode.value) { await axios.put(`/api/projects/${projectForm.id}`, projectForm); toast.success('Проект успешно обновлен!'); } else { await axios.post('/api/projects', projectForm); toast.success('Проект успешно создан!'); } closeProjectModal(); await fetchClients(); } catch (error) { handleError(error, 'проекта'); } };
