        ...
        <div v-else class="card">
            <div class="list-group list-group-flush">
                <a v-for="invoice in invoices" :key="invoice.id" :href="`/invoices/${invoice.id}`" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        <span class="fs-5 me-3">{{ invoice.total_amount }} {{ invoice.currency }}</span>
                ...
                <div class="modal-content">
                    <form @submit.prevent="handleCreateInvoice">
                        ...
                        <div class="modal-body">
                            ...
                            <!-- Шаг 2: Выбор записей времени -->
                            <div v-if="form.client_id" class="mb-3">
                                <label class="form-label">2. Выберите неоплаченные записи</label>
                                ...
                                <div v-else>
                                    <div class="input-group mb-2">
                                        <span class="input-group-text">Ставка за час (RUB)</span>
                                        <input type="number" class="form-control" v-model.number="form.unit_price" step="0.01" required>
                                    </div>
                            ...
                            <div v-if="form.time_entry_ids.length > 0" class="row">
                                <label class="form-label">3. Укажите даты</label>
...
const form = reactive({ client_id: '', time_entry_ids: [], unit_price: 1000, issue_date: new Date().toISOString().slice(0, 10), due_date: new Date(new Date().setDate(new Date().getDate() + 14)).toISOString().slice(0, 10), notes: null });
...
const fetchInvoices = async () => {
    try {
        ...
        alert("Произошла ошибка при загрузке счетов.");
    }
};
...
const loadUnbilledEntries = async () => {
    ...
    form.unit_price = selectedClient?.default_rate || 1000;
    ...
    try {
        ...
        console.error("Не удалось загрузить неоплаченные записи:", error);
    } finally {
...
const openCreateInvoiceModal = () => {
    Object.assign(form, { client_id: '', time_entry_ids: [], unit_price: 1000, issue_date: new Date().toISOString().slice(0, 10), due_date: new Date(new Date().setDate(new Date().getDate() + 14)).toISOString().slice(0, 10), notes: null });
    unbilled.entries = [];
